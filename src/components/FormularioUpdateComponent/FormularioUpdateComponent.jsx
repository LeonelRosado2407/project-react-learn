...
function FormularioUpdateComponent(){
    ...
    const usuarioId = location.pathname.split('/').reverse()[0];
    // console.log(usuarioId);

    const GetUSer = async ()=> {
        try {
            const response = await fetch(`http://localhost:3000/findUser/${usuarioId}`);
    ...
    const enviarDatos = ()=>{
        ...
        .then((data)=>{
            MySwal.fire({
                title: <p>Se ha actualizado Correctamente el Usuario: {nombre}</p>,
                icon: 'success',
                confirmButtonText: 'Ok',
        ...
        .catch((error)=>{
            MySwal.fire({
                title: <p>Error al crear al actualizar al usuario: {nombre}</p>,
                icon: 'error',
                confirmButtonText: 'Ok',
                confirmButtonColor: 'red'
            })
